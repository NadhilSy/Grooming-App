...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
    ...
    @Query(value = "SELECT SUM(total) FROM transaction",nativeQuery = true)
    Long getOmzet();
    ...
    @Query(value = "SELECT cus.user_id,cus.name,SUM(total) total_spent FROM transaction t JOIN user_account cus ON t.customer = cus.user_id GROUP BY cus.user_id,cus.name ORDER BY total_spent DESC LIMIT 3",nativeQuery = true)
